...
async def check_anomalies(
    ...
    commit_df = pd.DataFrame(commits_data)
    commit_df['SentimentScore'] = commit_df['CommitMessage'].apply(calculate_sentiment_score)
...
@app.post("/check_anomalies2/")
async def check_anomalies(
    repo: str,
    commits_data: List[dict],  # List of commit data (provided by the user)
    threshold: float = 0.5  # Base threshold for scaling
):
    # Check if the model exists
    repo_model_path = os.path.join(MODEL_DIR, f"{repo}_anomaly_model.joblib")
    if not os.path.exists(repo_model_path):
    ...
    # Dynamically calculate sentiment scores for commit messages
    commit_df['SentimentScore'] = commit_df['CommitMessage'].apply(calculate_sentiment_score)
    ...
    # Detect anomalies
    if isinstance(model, IsolationForest) or isinstance(model, OneClassSVM):
        raw_predictions = model.decision_function(features)  # Use decision function for scores
    elif isinstance(model, Sequential):
        predictions = model.predict(features)
        reconstruction_error = np.mean(np.abs(features - predictions), axis=1)
        raw_predictions = -reconstruction_error  # Lower error = more normal, higher = anomaly
    
    # Apply multi-level thresholds for classification
    commit_df["AnomalyLabel"] = "Normal"
    commit_df["Reason"] = None
    commit_df["AnomalyScore"] = raw_predictions
    ...
    for idx, row in commit_df.iterrows():
        ...
        if -0.5 <= score < -0.2:
            commit_df.at[idx, "AnomalyLabel"] = "Low Anomaly"
        elif -1.0 <= score < -0.5:
            commit_df.at[idx, "AnomalyLabel"] = "Moderate Anomaly"
        elif -1.5 <= score < -1.0:
            commit_df.at[idx, "AnomalyLabel"] = "High Anomaly"
        elif score < -1.5:
            commit_df.at[idx, "AnomalyLabel"] = "Critical Anomaly"
        
        # Rule-based explanations
        if row["Additions"] > commit_df["Additions"].mean() + 2 * commit_df["Additions"].std():
            reasons.append("High Additions")
        if row["Deletions"] > commit_df["Deletions"].mean() + 2 * commit_df["Deletions"].std():
            reasons.append("High Deletions")
        if row["SentimentScore"] < -0.5:
            reasons.append("Negative Sentiment")
        
        # Assign reasons if applicable
        if reasons:
            commit_df.at[idx, "Reason"] = ", ".join(reasons)
    
    anomalies = commit_df[commit_df["AnomalyLabel"] != "Normal"]
...
@app.post("/check_anomalies3/")
async def check_anomalies(
    repo: str,
    commits_data: List[dict],  # List of commit data (provided by the user)
    threshold_normal: float = -0.5,
    threshold_slight: float = -1.0,
    threshold_moderate: float = -1.5
):
    # Check if the model exists
    repo_model_path = os.path.join(MODEL_DIR, f"{repo}_anomaly_model.joblib")
    if not os.path.exists(repo_model_path):
    ...
    # Apply thresholds for graded classification
    commit_df["AnomalyLabel"] = "Normal"
    commit_df["Reason"] = None
    commit_df["AnomalyScore"] = raw_predictions
    ...
    for idx, row in commit_df.iterrows():
        ...
        # Assign graded anomaly labels
        if row["AnomalyScore"] < threshold_moderate:
            commit_df.at[idx, "AnomalyLabel"] = "Highly Anomalous"
        elif threshold_moderate <= row["AnomalyScore"] < threshold_slight:
            commit_df.at[idx, "AnomalyLabel"] = "Moderately Anomalous"
        elif threshold_slight <= row["AnomalyScore"] < threshold_normal:
            commit_df.at[idx, "AnomalyLabel"] = "Slightly Anomalous"
        
        # Reasons based on AnomalyScore and thresholds
        if row["AnomalyScore"] < threshold_moderate:
            reasons.append("Score indicates major deviation from the norm.")
        elif threshold_moderate <= row["AnomalyScore"] < threshold_slight:
            reasons.append("Score indicates moderate deviation from the norm.")
        elif threshold_slight <= row["AnomalyScore"] < threshold_normal:
            reasons.append("Score indicates slight deviation from the norm.")
        
        # Additional reasons based on other features
        if row["Additions"] > commit_df["Additions"].mean() + 2 * commit_df["Additions"].std():
            reasons.append("High Additions compared to average commits.")
        if row["Deletions"] > commit_df["Deletions"].mean() + 2 * commit_df["Deletions"].std():
            reasons.append("High Deletions compared to average commits.")
        if row["SentimentScore"] < -0.5:
            reasons.append("Negative sentiment in commit message.")
    ...
    return {
        ...
        "summary": {
            "Normal": len(commit_df[commit_df["AnomalyLabel"] == "Normal"]),
            "Slightly Anomalous": len(commit_df[commit_df["AnomalyLabel"] == "Slightly Anomalous"]),
            "Moderately Anomalous": len(commit_df[commit_df["AnomalyLabel"] == "Moderately Anomalous"]),
            "Highly Anomalous": len(commit_df[commit_df["AnomalyLabel"] == "Highly Anomalous"]),
        }
    }
